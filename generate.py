...
""".format(funcname=funcname))
  ...
  for argtype in argtypes:
    ...
    f.write("\t{0} arg{1} = PG_GETARG_{2}({3});\n".format(
        c_types[argtype], argvar, c_types[argtype].upper(), argnum))
    argnum += 1
...
def write_sql_function(f,
  ...
  f.write(
      "CREATE FUNCTION {sql_funcname}({argtypes}) RETURNS {rettype}"
      " IMMUTABLE{strict} LANGUAGE C AS '$libdir/moreint', '{funcname}';\n\n".
...
def write_sql_operator(f, funcname, leftarg, rightarg, op, rettype):
  ...
  if op == '%':
    ...
    sql_funcname = 'mod'
  write_sql_function(f,
                     funcname, [leftarg, rightarg],
                     rettype,
                     sql_funcname=sql_funcname)
...
def write_cmp_c_function(f, leftarg, rightarg):
  ...
  write_c_function(
      f, funcname, [leftarg, rightarg], 'int4', """if (arg1 > arg2)
\tresult = 1;
else if (arg1 == arg2)
\tresult = 0;
else
...
""".format(typ=typ,
           ctype=c_types[typ],
           Ctype=c_types[typ].replace('u', 'U').replace('i', 'I')))
...
def write_opclasses_sql(f, typ, pgversion):
  f.write("""CREATE OPERATOR CLASS {typ}_ops
    DEFAULT FOR TYPE {typ} USING btree FAMILY integer_ops AS
        ...
        OPERATOR        4       >= ,
        OPERATOR        5       > ,
        FUNCTION        1       bt{typ}{typ}cmp({typ}, {typ})""".format(
      typ=typ))
  if pgversion >= 9.2:
    f.write(""",
        FUNCTION        2       bt{typ}sortsupport(internal)""".format(
        typ=typ))
  f.write(""";
...
def write_code(f_c,
  ...
  write_op_c_function(f_c, funcname, leftarg, rightarg, op, rettype, c_check,
                      intermediate_type)
...
def write_arithmetic_op(f_c, f_sql, f_test_sql, op, leftarg, rightarg):
  args = sorted([leftarg, rightarg],
                key=lambda x: (type_bits(x), type_unsigned(x)))
  rettype = args[-1]
  ...
  if op == '*':
    if type_bits(rettype) < 64:
      ...
      if type_unsigned(leftarg) or type_unsigned(rightarg):
        ...
        c_check += ' && (arg2 != 0 && ((arg2 == -1 && arg1 < 0 && result < 0) || result / arg2 != arg1))'
  write_code(f_c, f_sql, leftarg, rightarg, op, rettype, c_check,
             intermediate_type)
  f_test_sql.write("""\
...
""".format(lefttype=leftarg, op=op, righttype=rightarg))
  if op in ['+', '*']:
    f_test_sql.write(
        "SELECT '{max_left}'::{lefttype} {op} '{max_right}'::{righttype};\n".
        format(lefttype=leftarg,
               op=op,
               righttype=rightarg,
  ...
  if op in ['/', '%']:
    f_test_sql.write("SELECT '5'::{lefttype} {op} '0'::{righttype};\n".format(
        lefttype=leftarg, op=op, righttype=rightarg))
  if op in ['%']:
    f_test_sql.write("SELECT mod('5'::{lefttype}, '2'::{righttype});\n".format(
        lefttype=leftarg, righttype=rightarg))
...
""".format(typ=argtype))
    if argtype in too_big:
      f_test_sql.write("SELECT '{num}'::{typ};\n".format(
          typ=argtype, num=max_values[argtype]))
      f_test_sql.write("SELECT '{num}'::{typ};\n".format(typ=argtype,
                                                         num=too_big[argtype]))
      if not type_unsigned(argtype):
        f_test_sql.write("SELECT '{num}'::{typ};\n".format(
            typ=argtype, num=min_values[argtype]))
        f_test_sql.write("SELECT '-{num}'::{typ};\n".format(
            typ=argtype, num=too_big[argtype]))
    f_test_sql.write("""\
...
""".format(typ=argtype,
           vals=(range(1, 6) if type_unsigned(argtype) else range(-2, 3))))
...
""".format(lefttype=leftarg, righttype=rightarg))
      ...
      for op in arithmetic_ops:
        write_arithmetic_op(f_c, f_sql, f_test_sql, op, leftarg, rightarg)
      f_test_sql.write("\n")

      if leftarg != rightarg:
        f_test_sql.write(
            "SELECT CAST('5'::{lefttype} AS {righttype});\n".format(
                lefttype=leftarg, righttype=rightarg))
        if type_bits(leftarg) > type_bits(rightarg) \
           or (type_bits(leftarg) == type_bits(rightarg) and type_unsigned(leftarg)):
          f_test_sql.write(
              "SELECT CAST('{num}'::{lefttype} AS {righttype});\n".format(
                  lefttype=leftarg, righttype=rightarg, num=too_big[rightarg]))
...
\tereport(ERROR,
\t\t(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
\t\t errmsg("{typ} out of range")));""".format(c_type=c_types[leftarg],
                                               typ=rightarg)
...
\t\t errmsg("{typ} out of range")));""".format(typ=rightarg)
        ...
        write_sql_function(f_sql,
                           c_funcname, [leftarg],
                           rightarg,
                           sql_funcname=sql_funcname)
        f_sql.write(
            "CREATE CAST ({lefttype} AS {righttype}) WITH FUNCTION {func}({arg}) AS {context};\n\n"
            .format(
                lefttype=leftarg,
                righttype=rightarg,
                func=sql_funcname,
...
""".format(typ=arg, op=op))
    ...
    if pgversion >= 9.2:
      write_sql_function(f_sql, 'bt' + arg + 'sortsupport', ['internal'],
                         'void')
...
""".format(funcname=funcname, typ=arg))
      f_sql.write(
          "CREATE AGGREGATE {agg}({typ}) (SFUNC = {sfunc}, STYPE = {stype}, SORTOP = {sortop});\n\n"
          .format(agg=agg, typ=arg, sfunc=funcname, stype=arg, sortop=op))
      f_test_sql.write(
          "SELECT {agg}(val::{typ}) FROM (VALUES (3), (5), (1), (4)) AS _ (val);\n\n"
          .format(agg=agg, typ=arg))

    for agg, funcname in [('bit_and', arg + arg + "and"),
                          ('bit_or', arg + arg + "or")]:
      f_sql.write(
          "CREATE AGGREGATE {agg}({typ}) (SFUNC = {sfunc}, STYPE = {stype});\n\n"
          .format(agg=agg, typ=arg, sfunc=funcname, stype=arg))
    f_test_sql.write(
        "SELECT bit_and(val::{typ}) FROM (VALUES (3), (6), (18)) AS _ (val);\n\n"
        .format(typ=arg))
    f_test_sql.write(
        "SELECT bit_or(val::{typ}) FROM (VALUES (9), (1), (4)) AS _ (val);\n\n"
        .format(typ=arg))
    ...
    f_sql.write(
        "CREATE AGGREGATE sum({arg}) (SFUNC = {sfunc}, STYPE = {stype});\n\n".
...
""".format(sfunc=sfunc, argtype=arg, stype=stype))
    ...
    f_sql.write(
        "CREATE AGGREGATE avg({arg}) (SFUNC = {sfunc}, STYPE = {stype}, FINALFUNC = int8_avg,"
        " INITCOND = '{{0,0}}');\n\n".format(arg=arg, sfunc=sfunc,
                                             stype=stype))
    f_test_sql.write("""
...
""".format(typ=lefttype, typ2=righttype))
      ...
      if lefttype != righttype:
        op_fam_btree_elements.extend([
            s.format(type1=lefttype, type2=righttype) for s in [
...
""")
  ...
  for leftarg, rightarg in (('int2', 'int4'), ('int4', 'int2'),
                            ('int8', 'int2'), ('int8', 'int4')):
    write_arithmetic_op(f_c, f_sql, f_test_sql, '%', leftarg, rightarg)
